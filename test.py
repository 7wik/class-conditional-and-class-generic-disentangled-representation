...
class fully_supervised_CONDVAE(nn.Module):
    def __init__(self):
        ...
        self.encoder = Encoder(
            ...
            [256 * 8 * 8, 2048 * 2],
            10,10,)
        self.encoder3 = Encoder3(self.w_dim)
        dec_fc = copy.copy([2048, 8 * 8 * 256])
        dec_fc.insert(0, 2*w_dim)
...
class Encoder(nn.Module):
    def __init__(self, filters, fc, w_dim, k):
        ...
        modules.append(fc_block(fc[-1], 2*w_dim,activation=False))
        ...
        self.net4 = torch.nn.Sequential(fc_block(128+k,2*self.w_dim,activation=False),)
...
def train():
    ...
    test_loader = torch.utils.data.DataLoader(
        torchvision.datasets.MNIST('/DATA1/satwik/data/mnist/', train=False, download=True,
    ...
    train_loader = torch.utils.data.DataLoader(
                            torchvision.datasets.MNIST('/DATA1/satwik/data/mnist/', train=True, download=True,transform=torchvision.transforms.Compose([
    ...
    for epoch in range(10):
        ...
        if not os.path.exists("/DATA1/satwik/xentropy/fmnist_fully_supervised"+str(self.w_dim)+"/"):
            os.makedirs("/DATA1/satwik/xentropy/fmnist_fully_supervised"+str(self.w_dim)+"/")
            print ("created the file--------------------------------------")
        torch.save(prob, "/DATA1/satwik/xentropy/fmnist_fully_supervised"+str(self.w_dim)+"/models")
        # break
train()
