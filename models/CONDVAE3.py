...
class CONDVAE3(nn.Module):
    def __init__(self, opts):
        super(CONDVAE3, self).__init__()
        self.k = 6
    ...
    def loss(self, data):
        ...
        klz = self.kld(mu[:, :-self.k * self.w_dim], logvar[:, :-self.k * self.w_dim], Variable(torch.cuda.FloatTensor([0.0])), Variable(torch.cuda.FloatTensor([0.0])))
        klz = torch.mean(klz)

        klw = self.kld(mu[:, -self.k * self.w_dim:], logvar[:, -self.k * self.w_dim:], Variable(torch.cuda.FloatTensor([0.0])), Variable(torch.cuda.FloatTensor([0.0])))
        klw = torch.mean(klw)
        ...
        for i in range(self.k):
            ...
            # logvar_i = logvar[:, -self.k * self.w_dim:][:, self.w_dim*i:self.w_dim*(i+1)]
            # p = - torch.sum(((w_i) ** 2)/ (2 * torch.exp(logvar_i)), -1)
        ...
        #     p2 = -0.5 * ( ((norm - 4)**2) / (sigma**2) + np.log(2 * np.pi) + 2 * np.log(sigma))
        #     pyIw += (1 - y[:,i]) * p1 + y[:,i] * p2

        pyIw = -torch.mean(pyIw)

        pyIz = - 1./(self.k + 1) * torch.sum(y_0, 1)
        # pyIz = - torch.sum(torch.exp(y_0) * y_0, 1)
    ...
    def kld(self, mu1, logvar1, mu2, logvar2):
        ...
        kld = 0.5 * torch.sum(-1 + logvar2 - logvar1 + ((mu2 - mu1)**2) / var2 + var1 / var2, -1)
        return kld
