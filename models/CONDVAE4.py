...
class CONDVAE4(nn.Module):
    def __init__(self, opts):
        super(CONDVAE4, self).__init__()
        self.k = 6
        ...
        self.decoder2 = Decoder2(opts['dec_filters'], opts['dec_fc'], opts['w_dim'], self.k)
    ...
    def decode2(self, z):
        return self.decoder2.forward(z)

    def forward(self, x, y):
        mu, logvar = self.encode(x.unsqueeze(1), y)
        z = self.sample(mu, logvar)
        y_0 = self.encode2(z[:, :-self.k * self.w_dim])
        y_1 = self.decode2(z[:, -self.k * self.w_dim:])

        return self.decode(z, y), mu, logvar, z, y_0, y_1

    def loss(self, data):
        x = data[0]
        y = data[2]

        recon_x, mu, logvar, samples, y_0, y_1 = self.forward(x, y)
        x = x.view(x.size(0), -1)
        ...
        for i in range(self.k):
            pyIw += y[:,i] * self.beta[0] * y_1[i][:,0] + (1 - y[:,i]) * self.beta[1] * y_1[i][:,1]
        pyIw = -torch.mean(pyIw)

        pyIz = - 1./(self.k + 1) * torch.sum(y_0, 1)
        pyIz = torch.mean(pyIz)

        l1 = self.alpha[0] * pxIzw + self.alpha[1] * klz + self.alpha[2] * klw + self.alpha[3] * pyIw
...
class Encoder(nn.Module):
    def __init__(self, filters, fc, w_dim, k):
        ...
        self.net3 = torch.nn.Sequential(
            ...
            fc_block(2048, 1024),
            fc_block(1024, 128),
            fc_block(128, k * w_dim + k, activation=False)
            )
    ...
    def forward(self, x, y):
        ...
        w2 = w[:,-k:].unsqueeze(2).repeat(1, 1, 2).view(-1, w_dim * k)

        return torch.cat([z[:,:z_split], w[:,:-k]], 1), torch.cat([z[:,z_split:], w2], 1)
...
class Decoder2(nn.Module):
    def __init__(self, filters, fc, w_dim, k):
        ...
        for i in range(self.k):
            self.net.append(torch.nn.Sequential(
                fc_block(w_dim, w_dim), 
                fc_block(w_dim, w_dim, activation=False), 
                nn.LogSoftmax(dim=1)
                ))
    ...
    def forward(self, w):
        l = []
        for i in range(self.k):
            l.append(self.net[i].forward(w[:, i*self.w_dim:(i+1)*self.w_dim]))
        return l